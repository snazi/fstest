...
description: "Converts architecture descriptions, module specs, or workflow docs into valid Mermaid diagram syntax for embedding in markdown. Use when the user asks to create a diagram, visualize architecture, generate a flowchart, sequence diagram, class diagram, state machine, entity relationship diagram (ERD), or Gantt chart. Supports flowcharts, sequence diagrams, class diagrams, state diagrams, ERDs, and Gantt charts. This skill does NOT cover Figma diagrams (use $figma instead) or image-based diagram generation."
---
...
When used inside a repository that has documentation or taxonomy rule files, read those rules first and treat them as the source of truth for environment names, output locations, and classification labels.
...
Document learnings in the repository's standard documentation or notes location when one is defined.
